...
if [ $TTN_STACK_VERSION -eq 2 ]; then
	...
	TC_TRUST=${TC_TRUST:-$(curl --silent "https://letsencrypt.org/certs/trustid-x3-root.pem")}
elif [ $TTN_STACK_VERSION -eq 3 ]; then
	TTN_REGION=${TTN_REGION:-"eu1"}
	TC_URI=${TC_URI:-"wss://${TTN_REGION}.cloud.thethings.network:8887"} 
	TC_TRUST=${TC_TRUST:-$(curl --silent "https://letsencrypt.org/certs/{trustid-x3-root.pem,isrgrootx1.pem}")}
else
